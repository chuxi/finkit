...
def daily_vol_rank(context: CollectorContext, mydate: date):
    ...
    with Session(engine) as session:
        session.execute(sal.text("delete from future_daily_vol_rank where date = :mydate"), {"mydate": mydate})
        session.commit()
...
def daily_kline(context: CollectorContext, mydate: date):
    ...
    if df is not None:
        ...
        engine.connect()
        with Session(engine) as session:
            session.execute(sal.text("delete from future_daily_kline where date = :mydate"), {"mydate": mydate})
            session.commit()
